...
void InputControl::update()
{
	...
	if (input_state.ThumbLX > 0.0f)
	{
		stick[1].x = (float)input_state.ThumbLX / (float)SHRT_MAX;
	}
	else
	{
		stick[1].x = -((float)input_state.ThumbLX / (float)SHRT_MIN);
	}
	if (input_state.ThumbLY > 0.0f)
	{
		stick[1].y = (float)input_state.ThumbLY / (float)SHRT_MAX;
	}
	else
	{
		stick[1].y = (float)input_state.ThumbLY / (float)SHRT_MIN;
	}
}
